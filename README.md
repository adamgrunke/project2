...
I was inspired to build this app after walking my dog in my neighborhood and constantly having to dodge the hazards on or near our path. Broken glass, questionable garbage, used needles, and dog droppings are just some of the things I would have to avoid during the walk. Unfortunately, I never had the proper tools to clean up some of these things on my walk. If only I had a tool that would mark the location so I could easily come back later with the proper cleanup kit. 

clunup starts by giving the user their location and allowing the user to mark that spot with the offending hazard. Other users in nearby can see the items as well so they can help out too. 
Users can currently add and delete hazards and tools from the app. There are also links to puchase tools and a place for safety information per hazard. Once the item is picked up, the user is able to delete it from the map. 


TECHNOLOGIES:
HTML, CSS, JAVASCRIPT, EXPRESS, NODE.JS, SEQUELIZE, POSTGRES, MAPBOX, NAVIGAOTR.GEOLOCATION

FUTURE DEVELOPEMENT:
Restrict users from deleting core hazards and tools. 
Isolate user added items to that specific user. 
Add points for identifying and cleaning up items. 
Show all map - clicking the points on the map will show the 

LINK TO LIVE SITE:


DEVELOPEMENT PROCESS/NOTES:
*====================================*
...
This has now been split into separate Hazard Settings and Tool Settings pages.
...
My original plan involved each type of hazard or tool being a boolean. Later I changed this so that the hazard and tool tables would only include type. 
| user	|
|:=====================:|
| id [pk]   | 	int     |
| email     |	string  |
